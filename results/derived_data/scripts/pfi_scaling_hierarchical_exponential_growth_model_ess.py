...
if __name__ == '__main__':
    ...
    for seed, n_ids in enumerate(n_ids_per_t):
        meas, t = generate_measurements(n_ids, pm, p, seed)
        logp = define_log_posterior(meas, t, mm)
        times += [estimate_evaluation_time(logp)]
    tofile = \
        directory + '/posteriors/hierarchical_exponential_growth_model' \
        '_pfi_eval_time.p'
    ...
    for seed, n_ids in enumerate(n_ids_per_t):
        ...
        tofile = \
            directory + '/posteriors/hierarchical_exponential_growth_model' \
            '_pfi_' + str(int(n_ids)) + '_samples.nc'

...
def run_inference(log_posterior, variances):
    ...
    controller._initial_params[:] = np.array([
        ...
        + np.random.normal(loc=0, scale=np.sqrt(variances), size=(3, 6))
...
def save_to_file(ess, posterior_stds, scales, names):
    # Create a dataframe
    df = pd.DataFrame(columns=['ESS'] + names)

    # Add data
    for ids, scale in enumerate(scales):
        variances = posterior_stds * scale
...
if __name__ == '__main__':
    ...
    posterior_stds = np.array([
        5.0e-5, 0.00012, 8.4e-7, 2.2e-6, 6.2e-5, 4.0e-5])
    scales = [1.4, 1.2, 1, 0.8, 0.6, 0.4, 0.2]
    ess = np.empty(shape=len(scales))
    for ids, scale in enumerate(scales):
        ess[ids] = run_inference(lp, posterior_stds * scale)

    save_to_file(
        ess, posterior_stds, scales, lp.get_parameter_names())
